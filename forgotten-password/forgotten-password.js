...
let error_message_el = document.getElementById("error-message");
let confirmation_message_el = document.getElementById("confirmation-message");

async function submit() {
    let selector_list = [".submit-buttons", ".error-message-text"];

    confirmation_message_el.style.display = "none";
    if (email_el.value.lenght === 0) {
        error_message_el.innerHTML = "Please enter an email address";
    ...
    if (response.status == 200) {
        confirmation_message_el.style.display = "block";
    } else if (response.status == 400 || response.status == 500) {
        let json = await response.json();
        error_message_el.innerHTML = json.message_fr; // TODO: display english messages
        error_message_el.style.display = "block";
    } else {
        alert("Unknown error");
    }
